...
class VoxelGrid():
    def __init__(self, size=128, bound_w=1):
        ...
        self.colors = Variable(torch.rand(
            (size*size*size, 3)).to(device), requires_grad=True)
        self.opacities = Variable(torch.rand(
            (size*size*size)).to(device), requires_grad=True)
    ...
    def unfold_arrays(self):
        unfolded_colors = self.colors.view(
            (self.size, self.size, self.size, 3)).transpose(2, 0)
        unfolded_opacities = self.opacities.view(
            (self.size, self.size, self.size)).transpose(2, 0)
    ...
    def subdivide(self):
        with torch.no_grad():
            ...
            new_colors = Variable(torch.zeros(
                (self.size*self.size*self.size, 3)).to(device), requires_grad=True)
            new_opacities = Variable(torch.zeros(
                (self.size*self.size*self.size)).to(device), requires_grad=True)
            ...
            # edges
            offsets = [np.array([1, 0, 0]), np.array(
            ...
            for off in offsets:
                ...
                new_colors[target_inds, :] = (
                    self.colors[c1_ind, :]+self.colors[c2_ind, :])/2.0
                new_opacities[target_inds] = (
                    self.opacities[c1_ind]+self.opacities[c2_ind])/2.0

            # Volume
            target_inds = self.flatten_3d_indices(
                2*old_indm+off[0]+off[1]+off[2])
            inds = []
            ...
            new_colors[target_inds, :] = sum(
                [self.colors[c_ind, :] for c_ind in inds])/8.0
            new_opacities[target_inds] = sum(
                [self.opacities[c_ind] for c_ind in inds])/8.0

            offsets = [(np.array([1, 0, 0]), np.array([0, 1, 0])), (np.array(
                [0, 1, 0]), np.array([0, 0, 1])), (np.array([0, 0, 1]), np.array([1, 0, 0]))]
    ...
    def render_rays(self, ordir_tuple, N_points, inv_depth=1.2):
        ...
        scatter_points = torch.rand_like(
            distances)*distances + torch.linspace(0, 10, N_points, device=device)[None, :]/inv_depth
        ...
        opacities = self.opacities[mesh_coords] * \
            in_bounds.float()  # not_in bounds: 0 opacity
    ...
    def render_image_from_rays(self, im_rays, kwargs):
        ...
        ori = torch.tensor(im_rays[0], dtype=torch.float32, device=device).view(
            (disp_im_w*disp_im_w, 3))
        direct = torch.tensor(im_rays[1], dtype=torch.float32, device=device).view(
            (disp_im_w*disp_im_w, 3))
    ...
    def render_large_image_from_rays(self, im_rays, kwargs, batch_size=1000):
        with torch.no_grad():
            ...
            for ori_it, direct_it in zip(ori.split(batch_size), direct.split(batch_size)):
                out_img[batch_size*ind:batch_size *
                        (ind+1)] = self.render_rays((ori_it, direct_it), *kwargs).detach()
                ind += 1

            out = out_img.view((disp_im_w, disp_im_w, 3)
                               ).cpu().detach().numpy()
...
class VoxelGridSpherical(VoxelGrid):
    def __init__(self, size=128, bound_w=1, num_harm=9):
        ...
        self.colors = Variable(torch.rand(
            (size*size*size, 3, num_harm)).to(device), requires_grad=True)
        self.opacities = Variable(torch.rand(
            (size*size*size)).to(device), requires_grad=True)
    ...
    def subdivide(self):
        with torch.no_grad():
            ...
            old_ind = regular_3d_indexes(self.size)
            self.size = 2 * self.size
            new_colors = Variable(torch.rand(
                (self.size*self.size*self.size, 3, self.num_harm)).to(device), requires_grad=True)
            new_opacities = Variable(torch.rand(
                (self.size*self.size*self.size)).to(device), requires_grad=True)
    ...
    def view_harmonics(self, points):
        ...
        phi = phi.cpu().numpy()
        harmonics = torch.zeros(
            (points.shape[0], self.num_harm), device=device)
        ind = 0
        for n in range(int(np.sqrt(self.num_harm))):
            for m in range(-n, n+1):
                Y = torch.tensor(
                    sph_harm(abs(m), n, theta, phi), device=device)
                if m < 0:
    ...
    def unfold_arrays(self):
        unfolded_colors = self.colors.view(
            (self.size, self.size, self.size, 3, self.num_harm)).transpose(2, 0)
        unfolded_opacities = self.opacities.view(
            (self.size, self.size, self.size)).transpose(2, 0)
        return unfolded_colors, unfolded_opacities
...
class VoxelGridInterp(VoxelGrid):
    ...
    def render_rays(self, ordir_tuple, N_points, inv_depth=1.2):
        ...
        cii = [ciii[i]*(1-diff[..., 1, None]) + ciii[i+2] *
               diff[..., 1, None] for i in [0, 1]]
        ...
        oii = [oiii[i]*(1-diff[..., 1]) + oiii[i+2]*diff[..., 1]
               for i in [0, 1]]
        opacities = (oii[0]*(1-diff[..., 2])+oii[1] *
                     diff[..., 2])*distances*in_bounds.float()
...
class VoxelGridCarve(VoxelGrid):
    ...
    def carve(self, ordir_tuple, N_points, inv_depth=1.2):
        with torch.no_grad():
            ...
            scatter_points = torch.linspace(0, 10, N_points, device=device)[
                None, :]/inv_depth
            p = ori + scatter_points[:, :, None]*(ordir_tuple[1][:, None, :])
    ...
    def color(self, ordir_tuple, pixels, N_points, inv_depth=1.2):
        with torch.no_grad():
            ...
            transp_term = torch.exp(-cumsum_opacities) * \
                (1-torch.exp(-opacities))

            self.colors[mesh_coords, :] += transp_term[...,
                                                       None]*pixels[:, None, :]
...
class VoxelGridSphericalCarve(VoxelGridSpherical):
    ...
    def color_sgd(self, ordir_tuple, pixels, N_points, lr=0.1, rand=0.003, inv_depth=1.2):
        with torch.no_grad():
            ...
            transp_term = (torch.exp(-cumsum_opacities) *
                           (1-torch.exp(-opacities)))[..., None, None]

            harmonics_base = self.view_harmonics(ordir_tuple[1])

            colors = self.colors[mesh_coords]
            est = (transp_term*colors *
                   harmonics_base[:, None, None, :]).sum(-1).sum(1)
            grad = (-2*transp_term *
                    harmonics_base[:, None, None, :])*((pixels-est)[:, None, :, None])
    ...
    def color(self, ordir_tuple, pixels, N_points, inv_depth=1.2):
        with torch.no_grad():
            ...
            self.colors[mesh_coords, :, 0] += transp_term[...,
                                                          None]*pixels[:, None, :]
            self.colors_sum[mesh_coords] += transp_term
    ...
    def smooth_colors(self):
        with torch.no_grad():
            ...
            for disp1 in [1, self.size, self.size**2]:
                new_ar[:-disp1] += self.colors[disp1:] * \
                    (self.opacities[disp1:] != 0)[..., None, None]
            ...
            for disp1 in [1+self.size, 1-self.size, 1+self.size**2, 1-self.size**2, self.size+self.size**2, self.size-self.size**2]:
                new_ar[:-disp1] += 1/np.sqrt(2)*self.colors[disp1:] * \
                    (self.opacities[disp1:] != 0)[..., None, None]
                opa_count[:-disp1] += 1 / \
                    np.sqrt(2)*(self.opacities != 0)[disp1:]
                new_ar[disp1:] += 1/np.sqrt(2)*self.colors[:-disp1] * \
                    (self.opacities[:-disp1] != 0)[..., None, None]
                opa_count[disp1:] += 1 / \
                    np.sqrt(2)*(self.opacities != 0)[:-disp1]
            for disp1 in [1+self.size+self.size**2, 1-self.size+self.size**2, 1+self.size-self.size**2, 1-self.size-self.size**2]:
                new_ar[:-disp1] += 1/np.sqrt(3)*self.colors[disp1:] * \
                    (self.opacities[disp1:] != 0)[..., None, None]
                opa_count[:-disp1] += 1 / \
                    np.sqrt(3)*(self.opacities != 0)[disp1:]
                new_ar[disp1:] += 1/np.sqrt(3)*self.colors[:-disp1] * \
                    (self.opacities[:-disp1] != 0)[..., None, None]
                opa_count[disp1:] += 1 / \
                    np.sqrt(3)*(self.opacities != 0)[:-disp1]
